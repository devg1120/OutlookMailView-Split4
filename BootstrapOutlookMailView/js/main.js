...
function loadMailItem(listItem) {
  ...
  $("#mail_attachments").html(
    mail.attachments.length > 0 ? "<ul>" + strAttachmentsHtml + "</ul>" : "",
  );
  $(".mail_content").show();
}
  ...
  for (var i = 0; i < mails.length; i++) {
    ...
    $mailItems.append(
      '<li class="item container" data-json=\'{{JSON}}\' data-sender-color="{{SENDER_COLOR}}" data-sender-image="{{SENDER_IMG}}">\
        ...
        .replace("{{SUMMARY}}", mails[i].summary.replace(/(<([^>]+)>)/gi, ""))
        .replace(
          "{{IMPORTANT}}",
          mails[i].isImportant
        ...
        .replace(
          "{{ATTACHMENT}}",
          mails[i].attachments.length > 0
            ? '<i class="ms-Icon ms-Icon--Attach"></i>'
            : "",
        )
        .replace(
          "{{REPLY}}",
          mails[i].isReplied ? '<i class="ms-Icon ms-Icon--ReplyAlt"></i>' : "",
        )
